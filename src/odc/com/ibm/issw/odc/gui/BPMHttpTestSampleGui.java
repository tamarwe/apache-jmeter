 ...
 *
 */
public class BPMHttpTestSampleGui extends AbstractSamplerGui {
    ...
    public BPMHttpTestSampleGui() {
        isAJP = false;
        init();
    }

    // For use by AJP
    protected BPMHttpTestSampleGui(boolean ajp) {
        isAJP = ajp;
        init();
    }
    ...
    public void configure(TestElement element) {
        ...
        String domain = samplerBase.getPropertyAsString(HTTPSamplerBase.DOMAIN);
        if("".equals(domain))
        {
        	samplerBase.setProperty(HTTPSamplerBase.DOMAIN, "${SERVER_HOST}");
        }
    ...
    public String getLabelResource() {
        return "odc_web_testing_title"; // $NON-NLS-1$
    }
